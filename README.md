...
Unreal Engine 5.7과 C++를 기반으로 제작한 3인칭 액션 RPG 프로젝트입니다.
...
`SandBoxMap`은 전투, AI, 보스 패턴, UI 기능을 개별 검증하는 테스트 맵입니다.
...
`UHawnmunAbilitySystemComponent`, `UHawnmunAttributeSet`, `UHawnmunGameplayAbility`를 중심으로 전투 상태를 관리합니다.
...
- `Ability.Rolling`, `Ability.Sprint`, `Ability.TargetLock`
- `State.Dead`, `State.Rolling`, `State.TargetLock`
- `Enemy.State.Boss.Phase2`
- 보스 패턴, 시네마틱, BGM, 아이템 태그

### 적 AI와 보스

`AHawnmunEnemy`가 일반 적의 GAS 초기화, Behavior Tree 실행, 어그로, 사망, 전리품과 리셋 흐름을 담당합니다.
...
`UHawnmunDungeonSubsystem`은 맵 전환 이후에도 유지되어야 하는 던전 세션 상태를 관리합니다.
...
`AHawnmunGameModeBase`는 레벨 이동, 저장과 로드, BGM, 어그로 적 초기화, 리스폰과 던전 결과 처리를 담당합니다.

### UI와 시네마틱

`AHawnmunHUD`가 Widget과 WidgetController를 생성하고 표시 상태를 관리합니다.
...
프로젝트에서는 GameplayAbilities, MotionWarping, PCG, Chaos Cloth Asset Editor, Movie Render Pipeline, Async Loading Screen 등의 플러그인을 사용합니다.

## 프로젝트 메모

- C++ 네이티브 컴포넌트 구조를 변경한 뒤 기존 Blueprint에 반영되지 않으면 Blueprint 상속 데이터가 남아 있을 수 있습니다. 컴포넌트 Reset, Reparent 또는 Blueprint 재생성을 검토합니다.
