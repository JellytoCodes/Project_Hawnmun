...
#pragma region Gameplay Effects Getter
...
#pragma region Gameplay Effects Setter
...
void UHawnmunFunctionLibrary::InitializeAttributesFromSaveData(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, UHawnmunSaveGame* SaveGame)
{
	if (!ASC || !SaveGame) return;

	const UHawnmunAttributeSet* AttrSet = ASC->GetSet<UHawnmunAttributeSet>();
	if (!AttrSet) return;

	ASC->SetNumericAttributeBase(UHawnmunAttributeSet::GetStrengthAttribute(), SaveGame->Strength);
	ASC->SetNumericAttributeBase(UHawnmunAttributeSet::GetIntelligenceAttribute(), SaveGame->Intelligence);
	ASC->SetNumericAttributeBase(UHawnmunAttributeSet::GetResilienceAttribute(), SaveGame->Resilience);
	ASC->SetNumericAttributeBase(UHawnmunAttributeSet::GetVigorAttribute(), SaveGame->Vigor);
	ASC->SetNumericAttributeBase(UHawnmunAttributeSet::GetHealthAttribute(), SaveGame->Health);
	ASC->SetNumericAttributeBase(UHawnmunAttributeSet::GetStaminaAttribute(), SaveGame->Stamina);
}
#pragma endregion
...
void UHawnmunFunctionLibrary::SetDeathImpulseDirection(FDamageEffectParams& DamageEffectParams, FVector ImpulseDirection, float Magnitude)
{
...
{
	checkf(DamageEffectParams.SourceAbilitySystemComponent, TEXT("ApplyDamageEffect failed: SourceAbilitySystemComponent is null."));
	checkf(DamageEffectParams.TargetAbilitySystemComponent, TEXT("ApplyDamageEffect failed: TargetAbilitySystemComponent is null."));
	checkf(DamageEffectParams.DamageGameplayEffectClass, TEXT("ApplyDamageEffect failed: DamageGameplayEffectClass is null."));
	checkf(DamageEffectParams.DamageType.IsValid(), TEXT("ApplyDamageEffect failed: DamageType is invalid."));

	const AActor* SourceAvatarActor = DamageEffectParams.SourceAbilitySystemComponent->GetAvatarActor();

	checkf(SourceAvatarActor, TEXT("ApplyDamageEffect failed: Source ASC has no AvatarActor."));
	...
	checkf(SpecHandle.IsValid() && SpecHandle.Data.IsValid(), TEXT("ApplyDamageEffect failed: could not create valid GameplayEffectSpec."));
