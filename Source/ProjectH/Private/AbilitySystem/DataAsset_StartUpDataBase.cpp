...
void UDataAsset_StartUpDataBase::InitializeStartupItem(UInventoryComponent* InInventoryComponent)
{
	check(InInventoryComponent);

	if (!StartUpItems.IsEmpty())
	{
		for (const FHawnmunStartUpItemInfo& ItemInfo : StartUpItems)
		{
			if (ItemInfo.ItemTag.IsValid())
			{
				InInventoryComponent->AddItem(ItemInfo.ItemTag, ItemInfo.ItemQuantity);
			}
		}
	}
}