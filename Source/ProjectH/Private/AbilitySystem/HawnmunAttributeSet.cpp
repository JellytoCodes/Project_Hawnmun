...
#pragma region Attribute Handlers
void UHawnmunAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	checkf(Props.TargetAvatarActor, TEXT("HandleIncomingDamage failed: TargetAvatarActor is null."));
	checkf(Props.TargetASC, TEXT("HandleIncomingDamage failed: TargetASC is null for %s."), *GetNameSafe(Props.TargetAvatarActor));
	checkf(Props.TargetCharacter, TEXT("HandleIncomingDamage failed: TargetCharacter is null for %s."), *GetNameSafe(Props.TargetAvatarActor));
	...
	if (bFatal)
	{
		...
		if (IEnemyInterface* Enemy = Cast<IEnemyInterface>(Props.TargetAvatarActor))
		{
			if (Enemy->TryPhaseTransition())
			{
				SetHealth(1.0f);
	...
	if (Props.TargetAvatarActor->Implements<UEnemyInterface>())
	{
		IEnemyInterface::Execute_SetAggroTarget(Props.TargetAvatarActor, Props.SourceAvatarActor);
	}
...
{
	checkf(Props.SourceASC, TEXT("HandleStamina failed: SourceASC is null. Stamina effects must have a valid source ASC."));
